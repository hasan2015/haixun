...
$(document).on("pageinit", "#p-mygrade", function() {
	...
	$("#navbar").on("click", function(e) {
		...
		if (oldTab != newTab) {
			...
			$(newTab).toggle();// 切换元素的可见状态
		}
		oldTab = newTab;
		$("#cset-year").trigger("updatelayout");
	});
...
function getYearOfMygrade() {
	...
	$
			.ajax({
				url : '/hxxk/page/getYearsOfMygrade.do',
				type : 'get', // 数据发送方式
				...
				success : function(msg) {
					...
					if (msg.code == RESULT_CODE_SUCCESS) {
						...
						var sections = [];
						var years = msg.result;
						for (var i = 0; i < years.length; i++) {
							var sid = years[i].year;
							sections
									.push('<section name="p1-section" data-role="collapsible" id="sec-'
											...
											+ '</ul></section>');
							// $("#lv-"+years[i].yearid).listview("refresh");

						}
						// alert(sections);
						$("#cset-year").empty();
						$("#cset-year").append(sections);
						$("#cset-year").trigger("create");
						// $("#cset-year").trigger("updatelayout");
						...
						// TODO 这是屏幕滚动，不是滑动
						/*
						 * $("h3,section").swipe( { //Generic swipe handler for
						 * all directions swipe : function(event, direction,
						 * distance, duration, fingerCount, fingerData) {
						 * //$(this).text("You swiped " + direction );
						 * alert(direction); }, //Default is 75px, set to 0 for
						 * demo so any distance triggers swipe threshold : 0 });
						 */
					} else {
						alert("获取数据失败：" + msg.result);
					}
				}
			});
}
// 根据年度，查找该年度的我的班级
function getMygradesByYear(year) {
	var datas = getSearchParams();
	datas.year = year;
	/*
	 * { "yearid" : yearid, "pageNow" : 0, "pageSize" : 10, "total" : 0 };
	 */
	// alert(datas.endtime);
	// $("#lv-mygrade").append('<li><a href="#">和平</a><p
	// class="hx-ui-li-aside">adsfdsa</p></li>');
	$.ajax({
		url : '/hxxk/page/getMygradesByYear.do',
		type : 'get', // 数据发送方式
		dataType : 'json', // 接受数据格式 (这里有很多,常用的有html,xml,js,json)
		traditional : true,
		data : datas, // 要传递的数据
...
//				alert("成功.msg=" + msg.result);
				...
				if (mygrades.length > 0) {
					for (var i = 0; i < mygrades.length; i++) {
						...
						var gid = mygrades[i].mygradeid;

						li.push('<li><a href="#" onclick="toPageGradeinfo('
								+ gid + ')" data-transition="slide">'
								+ mygrades[i].specialtyName +'('+mygrades[i].term+ ')</a><p id="p2-' + gid
								+ '" class="hx-ui-li-aside2">'
								+ mygrades[i].degree + '</p><p id="p-' + gid
								+ '" class="hx-ui-li-aside">'
								+ mygrades[i].gradeApplystatus + '</p></li>');
					...
					$("#lv-" + year).empty();
					$("#lv-" + year).append(li);
					$("#lv-" + year).listview("refresh");
					// $("#lv-" + yearid).trigger("updatelayout");
					for (var i = 0; i < mygrades.length; i++) {
						var gid = mygrades[i].mygradeid;
						//alert(mygrades[i].mygradeid);
						// 状态
						if (mygrades[i].gradeApplystatus == '待缴费')
							$("#p-" + gid).css("color", "#FF0000");// 纯红
						// 级别
...
function toPageGradeinfo(mygradeid) {
//	alert('mygradeid='+mygradeid)
	g_mygradeid = mygradeid;
	// $.mobile.changePage("#p-gradeinfo","{transition:'slide'}");
	$.mobile.changePage("mygradeinfo.jsp", "{transition:'slideup'}");
