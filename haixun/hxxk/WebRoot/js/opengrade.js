...
$(document).on("pageinit", "#p-opengrade", function() {
	//alert("pageinit");
	hideAllNavTab();
	$("#navbar").on("click", function(e) {
		// alert("nav-tabs.click");
		// hideAllNavTab();
		// $("div[@id^='tab']").hide();
		// $($(e.target).attr("href")).hide();
		// $(e.target).css("color", "#FF00FF");
		...
		// alert("section.click"+$(e.target).attr("href"));
	});
	// 选择“时间段”中，选择选项后“确定”
	/*
	 * $("#btn-date").on("click", function() {
	 * 
	 * //alert("str=" + selWeek.join(",")); $("#tab-date").hide();
	 * $("#cset-specialty").trigger("updatelayout"); });
	 */
...
$(document).on("pagebeforecreate", "#p-opengrade", function() {
	updateRefreshdate();
...
function getOpengradesBySpecialtyid(specialtyid) {
	 ...
	 */
	// alert(datas.endtime);
	// $("#lv-opengrade").append('<li><a href="#">和平</a><p
	...
	$.ajax({
		...
		success : function(msg) {
			// alert("成功.msg=" + msg.result.length);
			var li = [];
			var opengrades = msg.result;
			if (opengrades.length > 0) {
				for (var i = 0; i < opengrades.length; i++) {
					// var applystatus=opengrades[i].applystatus;
					var gid = opengrades[i].gradeid;

					li.push('<li><a href="#" onclick="toPageGradeinfo(' + gid
							+ ')" data-transition="slide">' + opengrades[i].code
							+ '</a><p id="p2-' + gid
							+ '" class="hx-ui-li-aside2">'
							+ opengrades[i].degree + '</p><p id="p-' + gid
							+ '" class="hx-ui-li-aside">'
							+ opengrades[i].applystatus + '</p></li>');
				...
				for (var i = 0; i < opengrades.length; i++) {
					var gid = opengrades[i].gradeid;
					// alert(opengrades[i].applystatus);
					// 状态
					if (opengrades[i].applystatus == '报名进行中')
						$("#p-" + gid).css("color", "#006600");// 绿色
					// 级别
					if (opengrades[i].degree == '高级')
						$("#p2-" + gid).css("color", "#FF0000");// 纯红
					else if (opengrades[i].degree == '中级')
						$("#p2-" + gid).css("color", "#FF1493");// 深粉
					else if (opengrades[i].degree == '初级')
						$("#p2-" + gid).css("color", "#FF00FF");// 紫红
					else if (opengrades[i].degree == '启蒙')
...
function getAllOpengrades() {
	...
	$.ajax({
		...
		success : function(msg) {
			...
			var opengrades = msg.result;
			for (var i = 0; i < opengrades.length; i++) {
				li.push('<li><a href="#">' + opengrades[i].specialtyName
			...
			// alert(li);
			$("#lv-opengrade").append(li);
			$("#lv-opengrade").listview("refresh");
			$("#lv-opengrade").trigger("updatelayout");
