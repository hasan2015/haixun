//在"去的"页面触发，在过渡动画开始前。
$(document).on("pagebeforeshow", "#p-gradeinfo", function() {
	updateRefreshdate();
	// alert("第二个界面的参数：" + getParameterByName('g-gradeid'));
	// alert(g_gradeid);
	getGradeinfoByGradeid(g_gradeid);
...
function getGradeinfoByGradeid(gradeid) {
	var datas = {
		"gradeid" : gradeid
	};
	$.ajax({
		url : '/hxxk/page/getGradeinfoByGradeid.do',
		type : 'get', // 数据发送方式
