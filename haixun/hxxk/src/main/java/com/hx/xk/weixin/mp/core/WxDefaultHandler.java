...
import com.hx.xk.weixin.mp.vo.PicInfo;
...
public class WxDefaultHandler implements WxHandler {
	...
	@Override
	public boolean check(String token, String signature, String timestamp,
			String nonce) throws AesException {

		if (null == signature || signature.length() > 128 || null == timestamp
				|| timestamp.length() > 128 || null == nonce
				|| nonce.length() > 128) {
			log.error("验证签名参数失败!!!");
			log.error("signature={},timestamp={},nonce={}", signature,
					timestamp, nonce);
			return false;
		}
		...
		if (log.isInfoEnabled()) {
			...
			log.info("signature={},timestamp={},nonce={}", signature,
					timestamp, nonce);
		}
	...
	public OutPutMsg voice(ReceiveMsg rm) {
		...
		if (log.isInfoEnabled()) {
			log.info("接收到音频消息...");
			log.info("msgid={}, from={}, to={}, mediaid={}, trans={}",
					rm.getMsgId(), rm.getFromUserName(), rm.getToUserName(),
					rm.getMediaId(), rm.getRecognition());
		}
		return om;
	}
	...
	public OutPutMsg video(ReceiveMsg rm) {
		...
		if (log.isInfoEnabled()) {
			log.info("接收到视频消息...");
			log.info("msgid={}, from={}, to={}, mediaid={}, thumbmid={}",
					rm.getMsgId(), rm.getFromUserName(), rm.getToUserName(),
					rm.getMediaId(), rm.getThumbMediaId());
		}
		return om;
	}
	...
	public OutPutMsg location(ReceiveMsg rm) {
		OutPutMsg om = new OutPutMsg(rm);
		om.setMsgType(WxMsgType.text.name());
		om.setContent("您当前的位置:" + rm.getLabel() + ",坐标:[" + rm.getLatitude()
				+ "," + rm.getLongitude() + "],地图缩放级别:" + rm.getScale());
		if (log.isInfoEnabled()) {
			log.info("接收到地理位置消息...");
			log.info(
					"msgid={}, from={}, to={}, x={}, y={}, scale={}, label={}",
					rm.getMsgId(), rm.getFromUserName(), rm.getToUserName(),
					rm.getLatitude(), rm.getLongitude(), rm.getScale(),
					rm.getLabel());
		}
		return om;
	}
	...
	public OutPutMsg eScanCodePush(ReceiveMsg rm) {
		OutPutMsg om = new OutPutMsg(rm);
		om.setMsgType(WxMsgType.text.name());
		String content = "您此次用二维码扫描菜单[" + rm.getEventKey() + "],扫描结果为: "
				+ rm.getScanResult();
		om.setContent(content);
		if (log.isInfoEnabled()) {
			log.info("接收到二维码扫描事件消息...");
			log.info(
					"msgid={}, from={}, to={}, eventKey={}, scantype={}, scanresult={}",
					rm.getMsgId(), rm.getFromUserName(), rm.getToUserName(),
					rm.getEventKey(), rm.getScanType(), rm.getScanResult());
		}
		return om;
	}
	...
	public OutPutMsg eScanCodeWait(ReceiveMsg rm) {
		OutPutMsg om = new OutPutMsg(rm);
		om.setMsgType(WxMsgType.text.name());
		String content = "您此次用扫描等待菜单[" + rm.getEventKey() + "],扫描结果为: "
				+ rm.getScanResult();
		om.setContent(content);
		if (log.isInfoEnabled()) {
	...
	public OutPutMsg ePicSysPhoto(ReceiveMsg rm) {
		OutPutMsg om = new OutPutMsg(rm);
		om.setMsgType(WxMsgType.text.name());
		String content = "您此次用系统拍照[" + rm.getEventKey() + "]共发了"
				+ rm.getCount() + "张图片,图片的MD5值为: ";
		...
		if (log.isInfoEnabled()) {
			...
			log.info(
					"msgid={}, from={}, to={}, eventkey={} count={}, picmd5sum={}",
					rm.getMsgId(), rm.getFromUserName(), rm.getToUserName(),
					rm.getEventKey(), rm.getCount(),
					String.valueOf(rm.getPicList()));
		}
		return om;
	}
	...
	public OutPutMsg ePicPhotoOrAlbum(ReceiveMsg rm) {
		OutPutMsg om = new OutPutMsg(rm);
		om.setMsgType(WxMsgType.text.name());
		String content = "您此次用拍照或相册[" + rm.getEventKey() + "]共发了"
				+ rm.getCount() + "张图片,图片的MD5值为: ";
		for (PicInfo pic : rm.getPicList()) {
			content += pic.getPicMd5Sum() + ", ";
		}
	...
	public OutPutMsg ePicWeixin(ReceiveMsg rm) {
		OutPutMsg om = new OutPutMsg(rm);
		om.setMsgType(WxMsgType.text.name());
		String content = "您此次用微信相册[" + rm.getEventKey() + "]共发了"
				+ rm.getCount() + "张图片,图片的MD5值为: ";
		for (PicInfo pic : rm.getPicList()) {
			content += pic.getPicMd5Sum() + ", ";
		}
	...
	public OutPutMsg eLocationSelect(ReceiveMsg rm) {
		...
		if (log.isInfoEnabled()) {
			...
			log.info("msgid={}, from={}, eventkey={}, to={}, "
					...
					rm.getMsgId(), rm.getFromUserName(), rm.getEventKey(),
					rm.getToUserName(), rm.getLatitude(), rm.getLongitude(),
	...
	public void eTemplateFinish(ReceiveMsg rm) {
		if (log.isInfoEnabled()) {
			...
			log.info("from={}, to={}, msgid={}, status={}",
					rm.getFromUserName(), rm.getToUserName(), rm.getMsgId(),
					rm.getStatus());
		}
	}
	...
	public void eSendJobFinish(ReceiveMsg rm) {
		if (log.isInfoEnabled()) {
			log.info("接收到群发推送消息...");
			log.info(
					"from={}, to={}, msgid={}, status={}, total={}, filter={}, sent={}, err={}",
					rm.getFromUserName(), rm.getToUserName(), rm.getMsgId(),
					rm.getStatus(), rm.getTotalCnt(), rm.getFilterCnt(),
					rm.getSentCnt(), rm.getErrorCnt());
		}
	}
	...
	public void eComponentVerifyTicket(ReceiveMsg rm) {
		if (log.isDebugEnabled()) {
			...
			log.info("appid={}, infotype={}, ticket={}", rm.getAppId(),
					rm.getInfoType(), rm.getTicket());
		}
	}
	...
	public void eUnAuthorizerMP(ReceiveMsg rm) {
		if (log.isDebugEnabled()) {
			...
			log.info("appid={}, infotype={}, unauthorizerappid={}",
					rm.getAppId(), rm.getInfoType(), rm.getUnAuthAppid());
		}
	}
