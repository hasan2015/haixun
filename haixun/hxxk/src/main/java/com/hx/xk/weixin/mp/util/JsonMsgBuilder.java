package com.hx.xk.weixin.mp.util;

import com.hx.xk.weixin.mp.commons.WxMsgType;
import com.hx.xk.weixin.mp.vo.Article;
...
public class JsonMsgBuilder {
	...
	public JsonMsgBuilder uploadNews(Article2... articles2s) {
		...
		for (Article2 art2 : articles2s) {
			art2_buf.append("{");
			art2_buf.append("\"thumb_media_id\":\"").append(art2.getMediaId())
					.append("\",");
			art2_buf.append("\"author\":\"").append(art2.getAuthor())
					.append("\",");
			art2_buf.append("\"title\":\"").append(art2.getTitle())
					.append("\",");
			art2_buf.append("\"content_source_url\":\"")
					.append(art2.getSourceUrl()).append("\",");
			art2_buf.append("\"content\":\"").append(art2.getContent())
					.append("\",");
			art2_buf.append("\"digest\":\"").append(art2.getDigest())
					.append("\",");
			art2_buf.append("\"show_cover_pic\":\"")
					.append(art2.getShowCover()).append("\"");
			art2_buf.append("},");
		}
	 ...
	 *            视频描述
	 */
	public JsonMsgBuilder uploadVideo(String mediaId, String title,
			String description) {
	...
	public JsonMsgBuilder sendAll(OutPutMsg msg) {
		if (msg.getToUsers().isEmpty()) {
			msgBuf.append("\"filter\":{").append("\"group_id\":\"")
					.append(msg.getGroupId()).append("\"},");
		} else {
		...
		switch (type) {
		case text:
			msgBuf.append("\"text\":{\"content\":\"").append(msg.getContent())
					.append("\"}");
			break;
		case image:
			msgBuf.append("\"image\":{\"media_id\":\"")
					.append(msg.getMediaId()).append("\"}");
			break;
		case voice:
			msgBuf.append("\"voice\":{\"media_id\":\"")
					.append(msg.getMediaId()).append("\"}");
			break;
		case mpvideo:
			msgBuf.append("\"mpvideo\":{\"media_id\":\"")
					.append(msg.getMediaId()).append("\"}");
			break;
		case video:
			msgBuf.append("\"video\":{\"media_id\":\"")
					.append(msg.getMediaId()).append("\",\"title\":\"")
					.append(msg.getTitle()).append("\",\"description\":\"")
					.append(msg.getDescription()).append("\"}");
			break;
		case mpnews:
			msgBuf.append("\"mpnews\":{\"media_id\":\"")
					.append(msg.getMediaId()).append("\"}");
			break;
		default:
			break;
