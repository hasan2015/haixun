...
import com.hx.xk.weixin.mp.MPSDK4J;
import com.hx.xk.weixin.mp.commons.WxEventType;
import com.hx.xk.weixin.mp.commons.WxMsgType;
import com.hx.xk.weixin.mp.util.StreamTool;
import com.hx.xk.weixin.mp.util.XMLHandler;
import com.hx.xk.weixin.mp.util.XmlMsgBuilder;
...
public class WxBase {
	...
	public String check() throws AesException {
		...
		boolean check = wxHandler.check(this.mpAct.getToken(), this.signature,
...
//			reply=WxUtil.getTransferCustomerService(rm);
		}
		return reply;
	}
...
//			om =wxHandler.eViewHb5(this.rm);
	...
	private ReceiveMsg convert2VO(InputStream msg)
		...
		// 调试信息
		if (log.isInfoEnabled()) {
			log.info("[MPSDK4J-{}]接收到微信消息[{},{}]:...", MPSDK4J.version(),
	 ...
	 * @throws AesException
	 */
	public String convert2XML(OutPutMsg msg) throws AesException {

		String reply_msg = "";

		// 自定义内容回复
		if (null != msg.getCustomReply() && !msg.getCustomReply().isEmpty()) {
		...
		// 调试信息
		if (log.isInfoEnabled()) {
			log.info("[MPSDK4J-{}]微信回复消息[{},{}]...", MPSDK4J.version(),
					msg.getMsgId(), msg.getCreateTime());
			log.info(reply_msg);
		}

		if (this.aesEncrypt) {// 加密
			reply_msg = this.wxInMsgCrt.encryptMsg(reply_msg, this.timeStamp,
					this.nonce);
		}
